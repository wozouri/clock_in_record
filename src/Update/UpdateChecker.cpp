...
void UpdateChecker::launchApplyScript(const QString& extractDir)
{
    ...
    const QString script = QStringLiteral(
        ...
        "}\n"
        "$files = @(Get-ChildItem -LiteralPath %1 -File)\n"
        "$main = @($files | Where-Object { $_.Name -ieq 'AttendanceApp.exe' })\n"
        "$rest = @($files | Where-Object { $_.Name -ine 'AttendanceApp.exe' })\n"
        "foreach ($f in ($main + $rest)) {\n"
        "    try { Copy-Item -LiteralPath $f.FullName -Destination (Join-Path %2 $f.Name) -Force -ErrorAction Stop }\n"
        "    catch { }\n"
        "}\n"
